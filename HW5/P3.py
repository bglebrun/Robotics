...
Z = np.zeros((100,2))
xSim = np.zeros((100,2,1))
xSim[0] = x0
#populate the Z matrix for use in calculations
while (k<100):
    procNoise = np.random.multivariate_normal(mu1,V,1)
    obNoise = np.random.multivariate_normal(mu2,np.array([[W,0.0],[0.0,W]]),1)
    xSim[k] = np.dot(F,xSim[k-1]) + np.array([[0], [2 * np.sin(k / 25)]]) + np.transpose(procNoise)
    Z[k] = np.dot(H,xSim[k]) + obNoise
    k = k+1
